...
public class TransmitRTP
{
	...
	public TransmitRTP( MediaLocator locator, String ipAddress, String port )
	{
	...
	private String createProcessor()
	{
		...
		for ( int i = 0; i < tracks.length; i++ ) {
			...
			if ( tracks[i].isEnabled() && format instanceof VideoFormat && !programmed ) {
				...
				new Dimension( w, h ), Format.NOT_SPECIFIED, Format.byteArray,
						frameRate );
	...
	// // an RTP transmitter. The other way is to use the RTPSessionManager
	// API.
				...
				if ( owner instanceof Codec ) {
					...
					for ( int j = 0; j < fmts.length; j++ ) {
						if ( fmts[j].matches( jpegFmt ) ) {
							...
							qc.setQuality( val );
							System.err.println( "- Setting quality to " + val + " on "
									+ qc );
							break;
						}
					}
	...
	public static void main( String[] args )
	{
		...
		// if ( args.length < 3 ) {
		// System.err.println( "Usage: VideoTransmit <sourceURL> <destIP>
		// <destPort>" );
		// System.exit( -1 );
		// }

		// Create a video transmit object with the specified params.
		TransmitRTP vt = new TransmitRTP( new MediaLocator(
				"file://Users/mat/Music/sample.mp4" ), "127.0.0.255", "42050" );
		// Start the transmission
